...
router.get("/cocktails_list/:cocktailId", (req, res) => {
  ...
  let cocktail_recipes_data = fs.readFileSync("data/cocktail_recipes.json");
  let parse_cocktail_recipes_data = JSON.parse(cocktail_recipes_data);
  let currentCocktailDetails = parse_cocktail_recipes_data.find(
...
router.get("/taste_preferences_list", (req, res) => {
  let taste_preferences_list_data = fs.readFileSync(
    "data/taste_preferences_list.json"
  );
  let parse_taste_preferences_list_data = JSON.parse(
    taste_preferences_list_data
  );
...
router.get("/represent_mood_list", (req, res) => {
  let represent_mood_list_data = fs.readFileSync(
    "data/represent_mood_list.json"
  );
...
router.post("/cocktails_list", (req, res) => {
  ...
  let stringify_cocktail_recipes_data = JSON.stringify(
    parse_cocktail_recipes_data
  );
  fs.writeFileSync(
    "data/cocktail_recipes.json",
    stringify_cocktail_recipes_data
  );
